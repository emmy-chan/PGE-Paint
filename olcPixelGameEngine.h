...
namespace olc
{
	...
	float clamped(float i, float min, float max) {
		if (i < min)
			i = min;
	...
	static olc::Pixel Rainbow(float freq)
	{
		static uint32_t cnt = 0;

		olc::Pixel color = olc::Pixel(
			(int)sin(freq * cnt + 0) * 127 + 128,
			(int)sin(freq * cnt + 2) * 127 + 128,
			(int)sin(freq * cnt + 4) * 127 + 128,
			255);
	...
	{
		hue = clamped(hue, 0.0f, 360.0f);
		saturation = clamped(saturation, 0.0f, 1.0f);
		value = clamped(value, 0.0f, 1.0f);
	...
	public: // DRAWING ROUTINES
		...
		void FillRectFade(const olc::vi2d& pos, const olc::vi2d& size, Pixel p, bool horizontal = false);
		void FillRectFade(int32_t x, int32_t y, int32_t w, int32_t h, Pixel p, bool horizontal = false);
		...
		void DrawDecal(const float x, const float y, olc::Decal* decal, const olc::vf2d& scale = { 1.0f,1.0f }, const olc::Pixel& tint = olc::WHITE);
		// Draws a region of a decal, with optional scale and tinting
		void DrawPartialDecal(const float x, const float y, olc::Decal* decal, const olc::vf2d& source_pos, const olc::vf2d& source_size, const olc::vf2d& scale = { 1.0f,1.0f }, const olc::Pixel& tint = olc::WHITE);
		void DrawPartialDecal(const float x, const float y, const olc::vf2d& size, olc::Decal* decal, const olc::vf2d& source_pos, const olc::vf2d& source_size, const olc::Pixel& tint = olc::WHITE);
		...
		void DrawStringDecal(float x, float y, const std::string& sText, const Pixel col = olc::WHITE, const olc::vf2d& scale = { 1.0f, 1.0f });
		void DrawStringPropDecal(const int x, const int y, const std::string& sText, const Pixel col = olc::WHITE, const olc::vf2d& scale = { 1.0f, 1.0f });
		...
		void DrawStringShadow(const olc::vi2d& pos, const std::string& sText, Pixel col = olc::WHITE, uint32_t scale = 1);
		void DrawStringShadowCenter(olc::vi2d& pos, const std::string& sText, Pixel col, uint32_t scale);
		void DrawStringCenter(int x, int y, const std::string& sText, Pixel col, uint32_t scale = 1);
		void DrawHealthBar(int x, int y, int size, float health, float maxHealth, Pixel colorBar, Pixel colorBG);
		void DrawHealthBarVert(int x, int y, int size, float health, float maxHealth, Pixel colorBar, Pixel colorBG);
		void DrawHealthBarLerp(int x, int y, int size, float& health, float actualHealth, float maxHealth, float fElapsedTime, Pixel colorBar, Pixel colorBG);
...
namespace olc
{
	...
	olc::rcode PixelGameEngine::Start()
	{
		...
		// Construct the window (sets position to middle - emmy)
		if (platform->CreateWindowPane({ GetSystemMetrics(SM_CXSCREEN) / 2 - vScreenSize.x, GetSystemMetrics(SM_CYSCREEN) / 2 - vScreenSize.y }, vWindowSize, bFullScreen) != olc::OK) return olc::FAIL;
	...
	void PixelGameEngine::FillRect(int32_t x, int32_t y, int32_t w, int32_t h, Pixel p)
	{
		...
		if (x < 0) x = 0;
		else if (x >= (int32_t)GetDrawTargetWidth()) x = (int32_t)GetDrawTargetWidth();
		if (y < 0) y = 0;
		else if (y >= (int32_t)GetDrawTargetHeight()) y = (int32_t)GetDrawTargetHeight();

		if (x2 < 0) x2 = 0;
		else if (x2 >= (int32_t)GetDrawTargetWidth()) x2 = (int32_t)GetDrawTargetWidth();
		if (y2 < 0) y2 = 0;
		else if (y2 >= (int32_t)GetDrawTargetHeight()) y2 = (int32_t)GetDrawTargetHeight();
	...
	void PixelGameEngine::FillRectFade(const olc::vi2d& pos, const olc::vi2d& size, Pixel p, bool horizontal)
	{
		FillRectFade(pos.x, pos.y, size.x, size.y, p, horizontal);
	}

	void PixelGameEngine::FillRectFade(int32_t x, int32_t y, int32_t w, int32_t h, Pixel p, bool horizontal)
	{
		int32_t x2 = x + w;
		int32_t y2 = y + h;
		...
		for (int i = 0; x + i < x2; i++)
			for (int j = 0; y + j < y2; j++)
				Draw(x + i, y + j, Pixel(p.r, p.g, p.b, p.a - (j * (p.a - 40) / (y2 - y)))); //40 = fade end

		/*if (horizontal) {
			for (int i = 0; x + i < x2; i++)
				for (int j = 0; y + j < y2; j++)
					Draw(x + i, y + j, Pixel(p.r, p.g, p.b, p.a - (i * (p.a - 40) / (x2 - x))));
		}
		else {
			for (int i = 0; x + i < x2; i++)
				for (int j = 0; y + j < y2; j++)
					Draw(x + i, y + j, Pixel(p.r, p.g, p.b, p.a - (j * (p.a - 40) / (y2 - y))));
		}*/
	}
	...
	void PixelGameEngine::DrawSprite(int32_t x, int32_t y, Sprite* sprite, uint32_t scale, uint8_t flip)
	{
		...
		static int32_t fxs = 0, fxm = 1, fx = 0;
		static int32_t fys = 0, fym = 1, fy = 0;

		if (flip) {
			if (olc::Sprite::Flip::HORIZ) { fxs = sprite->width - 1; fxm = -1; }
			else if (olc::Sprite::Flip::VERT) { fys = sprite->height - 1; fym = -1; }
		}
	...
	void PixelGameEngine::DrawPartialSprite(int32_t x, int32_t y, Sprite* sprite, int32_t ox, int32_t oy, int32_t w, int32_t h, uint32_t scale, uint8_t flip)
	{
		...
		if (flip) {
			if (olc::Sprite::Flip::HORIZ) { fxs = w - 1; fxm = -1; }
			else if (olc::Sprite::Flip::VERT) { fys = h - 1; fym = -1; }
		}
	...
	void PixelGameEngine::DrawPartialDecal(const float x, const float y, olc::Decal* decal, const olc::vf2d& source_pos, const olc::vf2d& source_size, const olc::vf2d& scale, const olc::Pixel& tint)
	{
		olc::vf2d vScreenSpacePos =
		{
			(std::floor(x) * vInvScreenSize.x) * 2.0f - 1.0f,
			((std::floor(y) * vInvScreenSize.y) * 2.0f - 1.0f) * -1.0f
		};
	...
	void PixelGameEngine::DrawPartialDecal(const float x, const float y, const olc::vf2d& size, olc::Decal* decal, const olc::vf2d& source_pos, const olc::vf2d& source_size, const olc::Pixel& tint)
	{
		olc::vf2d vScreenSpacePos =
		{
			(std::floor(x) * vInvScreenSize.x) * 2.0f - 1.0f,
	...
	void PixelGameEngine::DrawDecal(const float x, const float y, olc::Decal* decal, const olc::vf2d& scale, const olc::Pixel& tint)
	{
		olc::vf2d vScreenSpacePos =
		{
			(std::floor(x) * vInvScreenSize.x) * 2.0f - 1.0f,
	...
	void PixelGameEngine::DrawStringDecal(float x, float y, const std::string& sText, const Pixel col, const olc::vf2d& scale)
	{
		...
		for (auto c : sText)
		{
			...
			else
			{
				int32_t ox = (c - 32) % 16;
				int32_t oy = (c - 32) / 16;
				DrawPartialDecal(x + spos.x, y + spos.y, fontDecal, { float(ox) * 8.0f, float(oy) * 8.0f }, { 8.0f, 8.0f }, scale, col);
				x += 8.0f * scale.x;
			}
		}
	}

	void PixelGameEngine::DrawStringPropDecal(const int x, const int y, const std::string& sText, const Pixel col, const olc::vf2d& scale)
	{
		olc::vf2d spos = { 0.0f, 0.0f };
		for (auto c : sText)
		{
			...
			else
			{
				int32_t ox = (c - 32) % 16;
				int32_t oy = (c - 32) / 16;
				DrawPartialDecal(x + spos.x, y + spos.y, fontDecal, { float(ox) * 8.0f + float(vFontSpacing[c - 32].x), float(oy) * 8.0f }, { float(vFontSpacing[c - 32].y), 8.0f }, scale, col);
	...
	void PixelGameEngine::DrawStringShadow(const olc::vi2d& pos, const std::string& sText, Pixel col, uint32_t scale)
	{
		DrawStringDecal(pos.x - 1, pos.y + 1, sText, olc::BLACK); //, scale
		DrawStringDecal(pos.x, pos.y, sText, col); //, scale
	}

	void PixelGameEngine::DrawStringShadowCenter(olc::vi2d& pos, const std::string& sText, Pixel col, uint32_t scale)
	{
		olc::vi2d size = GetTextSize(sText);
		pos.x -= size.x / 2;

		DrawStringShadow(pos, sText, col);
	}

	void PixelGameEngine::DrawStringCenter(int x, int y, const std::string& sText, Pixel col = olc::WHITE, uint32_t scale)
	{
		olc::vi2d size = GetTextSize(sText);

		x -= size.x / 2;

		DrawStringDecal(x, y, sText, col); //, scale
	}
	...
	void PixelGameEngine::DrawHealthBar(int x, int y, int size, static float health, float maxHealth, Pixel colorBar, Pixel colorBG) {
		if (health > maxHealth)
			health = maxHealth;

		FillRect(x, y, size + 1, 3, colorBG);
		FillRect(x + 1, y + 1, health * size / maxHealth - 1, 1, colorBar);
	}

	void PixelGameEngine::DrawHealthBarVert(int x, int y, int size, static float health, float maxHealth, Pixel colorBar, Pixel colorBG) {
		if (health > maxHealth)
			health = maxHealth;

		FillRect(x, y, 3, size + 1, colorBG);

		for (int i = 1; i < health * size / maxHealth; i++) {
			Draw(x + 1, y + size - i, colorBar);
		}

		//FillRect(x + 1, y + 1, 1,  - 1, colorBar);
	}

	void PixelGameEngine::DrawHealthBarLerp(int x, int y, int size, static float& health, float actualHealth, float maxHealth, float fElapsedTime, Pixel colorBar, Pixel colorBG) {
		health = lerp(health, actualHealth, 12.5f * fElapsedTime);
		...
		for (int i = 0; i < health * size / maxHealth - 1; i++) {
			if (colorBar.g < 255)
				colorBar.g += 1;
...
namespace olc
{
	...
	public:
		...
		virtual olc::rcode CreateWindowPane(const olc::vi2d& vWindowPos, olc::vi2d& vWindowSize, bool bFullScreen) override
		{
			...
			wc.lpszClassName = olcT("OpenGL"); //OLC_PIXEL_GAME_ENGINE fuck plugging olc
			RegisterClass(&wc);
			...
			DWORD dwStyle = WS_VISIBLE | WS_POPUP; //hide window frame - emmy
			...
			olc_hWnd = CreateWindowEx(dwExStyle, olcT("OpenGL"), olcT(""), dwStyle, //dwExStyle, olcT("OLC_PIXEL_GAME_ENGINE") //fuck plugging olc
